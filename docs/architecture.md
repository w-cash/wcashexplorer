...
consensus and does not replace validation by a Wcash node. Its job is to
preserve the exact public data it receives, independently validate the Wcash
AuxPoW envelope, track canonical selection safely through reorganizations, and
state precisely which optional external observations support each claim.

```text
                              private network
  Wcash Testnet RPC ────────────────┐
  optional Zcash RPC observers ─────┴─> Rust indexer / AuxPoW verifier ─> PostgreSQL
                                                                              │
...
indexing, it checks the node's chain name and genesis hash. When Zcash parent
observers are configured, it also validates them against the expected Zcash
Testnet chain name and genesis hash.
...
`unavailable`, `not_configured`, and `disagreement`. With no Zcash observer, the
primary deployment records `not_configured`; the Wcash proof remains locally
verifiable, but the explorer makes no Zcash-canonical claim.
`parentSourcesAgree` means the configured observations have the same state.
Even with distinct URLs, they may share an operator, network path, or
implementation, so this is not a Byzantine quorum or a cryptographic finality
proof.
...
Exact-witness state, and any configured parent confirmations and canonical
status, are observations rather than immutable block facts. The indexer
periodically revisits the stalest AuxPoW record inside a bounded recent window.
This updates the exact Wcash witness state and optional parent observations
...
- **No parent RPC configured:** local Wcash AuxPoW verification continues and
  parent observation is explicitly `not_configured`.
- **One optional parent RPC unavailable:** the per-source state records the
  failure; aggregate agreement can no longer be claimed.
- **Optional parent disagreement:** the block's local proof may remain valid
  while the parent observation is explicitly `disagreement`.
